...
void Mesh::asBox(double width, double height, double depth)
{
    ...
    m_vertices[10].texcoord = Vec2(1.f, 0.f);
...
{
    Mesh *mesh = new Mesh();
    mesh->asFloor(length, height);
    return mesh;
}

Mesh *Mesh::CreateTriangle(const Vec3 p1, const Vec3 p2, const Vec3 p3)
{
    Mesh *mesh = new Mesh();
    mesh->asTriangle(p1, p2, p3);
    return mesh;
}
